...
1. **File `.env` memang belum ada & tidak akan kamu temukan di GitHub** —
   itu disengaja demi keamanan (isinya rahasia). Buat sendiri:
   - Caranya: salin file **`.env.example`** → gandakan/duplikat →
     ubah nama hasil salinannya menjadi **`.env`** (tanpa ".example",
     persis `.env`, tanpa akhiran apa pun).
   - Atau buat file baru kosong bernama `.env` lalu isi manual seperti
     di bawah.
2. Isi file `.env` itu dengan connection string Neon-mu:

   ```
   DATABASE_URL=<tempel connection string Neon dari Langkah 2>
   KOMTING_PIN=komting123
   ```
...
4. Buka lagi websitemu → di Dashboard klik **"Muat Data Contoh"** kalau
